...
export function ReportView({ report, showDerivation = true }: { report: Report; showDerivation?: boolean }) {
  return <div className="topic-report">
    ...
    {showDerivation && <section className="report-card derivation" aria-label="图解推导"><h3><BookOpen size={19} /> 图解推导</h3><ol>{report.steps.map((step, i) => <li key={i}><p>{step}</p>{report.diagrams?.[i] && <DiagramView diagram={report.diagrams[i]} />}</li>)}</ol></section>}
